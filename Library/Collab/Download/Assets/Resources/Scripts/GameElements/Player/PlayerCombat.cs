...
namespace LimoKnight
{
    ...
    public class PlayerCombat : MonoBehaviour
    {
        ...
        public void SetActualStats(int maxHp, int damage, int damageReduction, int criticalChance, float luckAfterBattle)
        {
            _maxHP = maxHp; _damage = damage; _damageReduction = damageReduction; _criticalChance = criticalChance; _luckAfterBattle = luckAfterBattle;
            if (_maxHP <= 0) return;
            if (_actualHP == -1)
                _actualHP = _maxHP;
        ...
        private IEnumerator DieCoroutine()
        {
            ...
            Debug.LogError("PLAYER ANIMATOR > DIE MUST TRIGGER - START&END ANIMATION");
            _animator.Play("Die");
            yield return new WaitUntil(() => _isDoingAnimation == false);
            StatsManager.Instance.RefreshHP(_actualHP, _maxHP);
        ...
        {
            yield return new WaitUntil(() => _isDoingAnimation == false);
            Debug.LogError("PLAYER ANIMATOR > RECIEVE ATTACK MUST TRIGGER - START&END ANIMATION");
